...
REGION_CODE_BY_COUNTRY: dict[str, dict[str, str]] = {
    "CA": {
...
def _region_code(country_code: str, region_name: str, explicit_code: Any = None) -> str:
    explicit = _clean_text(explicit_code).upper()
    if explicit:
        return explicit
    country = _clean_text(country_code).upper()
    return REGION_CODE_BY_COUNTRY.get(country, {}).get(_clean_text(region_name).lower(), "")


def city_geography_fields(geocode: dict[str, Any]) -> dict[str, str]:
    country_name = _clean_text(geocode.get("country_name") or geocode.get("country"))
    country_code = _clean_text(geocode.get("country_code")).upper()
    region_name = _clean_text(geocode.get("region_name") or geocode.get("admin1")) or "Unspecified"
    region_code = _region_code(country_code, region_name, geocode.get("region_code"))
    return {
...
def summarize_city_weather(city_name: str, geocode: dict[str, Any], daily_payload: dict[str, Any], gdd_base_c: float) -> tuple[dict[str, Any], list[dict[str, Any]], dict[str, Any]]:
    ...
    geography = city_geography_fields(geocode)
...
def forecast_rows(city_name: str, payload: dict[str, Any], model: str, geocode: dict[str, Any] | None = None) -> list[dict[str, Any]]:
    daily = payload.get("daily") or {}
    times = daily.get("time") or []
    run_timestamp = datetime.now(timezone.utc).isoformat()
    geography = city_geography_fields(geocode or payload) if geocode else {}
    rows = []
    for i, raw_day in enumerate(times):
        rows.append({
            "city_name": city_name,
