...
test('desktop updater download progress is marked and guarded', () => {
	...
	assert.match(source, /let trellisUpdateDownloadInProgress = false;/, 'download progress should track active downloads');
	assert.match(source, /if \(trellisUpdateDownloadInProgress\) return;/, 'repeat update downloads should be ignored while active');
	assert.match(source, /if \(silentUpdate\) return;/, 'silent updates should keep suppressing visible progress UI');
	assert.match(source, /if \(silentUpdate \|\| !trellisUpdateDownloadInProgress\) return;/, 'downloaded updates should only prompt after a visible accepted download');
});

test('desktop updater progress starts indeterminate then switches to percent progress', () => {
	const source = readElectronMain();

	assert.match(source, /function beginTrellisUpdateDownload\(\)[\s\S]*trellisUpdateProgressBar = createTrellisUpdateProgressBar\(\);/, 'downloads should show an immediate indeterminate progress dialog');
	assert.match(source, /autoUpdater\.on\('download-progress', showTrellisUpdateDownloadProgress\)/, 'download progress events should drive the progress dialog');
	...
	assert.match(source, /progressBar\.value = percent;/, 'determinate progress should update the rendered progress value');
	assert.match(source, /trellisUpdateProgressBar\.detail = detail;[\s\S]*trellisUpdateProgressBar\.value = percent;/, 'subsequent progress events should update detail text and value');
});
...
test('desktop updater progress error path avoids private progressbar internals', () => {
	...
	assert.match(source, /closable: true,[\s\S]*text: 'Trellis Studio update failed\.'/, 'download errors should show a closable error dialog');
	assert.match(source, /updateDownload\.catch\(showTrellisUpdateDownloadError\);/, 'download promise failures should update the progress dialog error state');
