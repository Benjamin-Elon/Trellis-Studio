...
const {
    ...
    openDatabase,
    runDiagnostics
} = require('../scripts/trellis_seed_sowing_window_diagnostics.cjs');
...
test('sowing-window diagnostic reports scheduler mismatch without failing', () => {
...
test('sowing-window diagnostic fails setup for unresolved dependencies', () => {
    withTempDb((db, dbPath) => {
...
test('sowing-window diagnostic fails setup for malformed reference rows', () => {
    withTempDb((db, dbPath) => {
