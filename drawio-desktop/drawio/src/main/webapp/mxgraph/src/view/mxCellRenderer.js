...
/**
 * Function: inheritFontStyle
 * 
 * Inherits the given text style from the parent cell.
 */
mxCellRenderer.prototype.inheritFontStyle = function(state, field, key)
{
	if (state.text != null && state.text[field] == 'inherit')
	{
		var graph = state.view.graph;
		var pstate = graph.getView().getState(
			graph.model.getParent(state.cell));
		
		if (pstate != null)
		{
			state.text[field] = pstate.style[key];
		}
	}
};
...
mxCellRenderer.prototype.createTextShape = function(state, value, dialect)
{
	...
	text.convertToSvg = mxUtils.getValue(state.style, 'convertToSvg', '0') == '1';
...
mxCellRenderer.prototype.redrawLabel = function(state, forced)
{
	...
	var convertToSvg = mxUtils.getValue(state.style, 'convertToSvg', '0') == '1';
	...
	if (state.text != null && (state.text.wrap != wrapping || state.text.clipped != clipping ||
		...
		state.text.convertToSvg != convertToSvg))
	{
		state.text.destroy();
		state.text = null;
	}
	...
	if (state.text != null)
	{
		...
		this.inheritFontStyle(state, 'size', mxConstants.STYLE_FONTSIZE);
		this.inheritFontStyle(state, 'family', mxConstants.STYLE_FONTFAMILY);
